...
function sendMessage() {
    ...
    if ($.trim(messageInput.val()).length > 0 || hasFile) {
        ...
        formData.append("channel", selectedChannel.uniqueName);
...
function makeSeen(status) {
    ...
    $.ajax({
        ...
        dataType: "JSON",
        success: (data) => {
            console.log(data,"[seen] Messages seen - " + messenger.split("_")[1],data.author, messenger.split("_")[1]);
            if (data.status == 1) {
                console.log(" ------------ " + messenger.split("_")[1]);
...
$(document).ready(function () {
    ...
    checkInternet('connecting', selector);
    ...
    function connectMessagingClient(token) {
        ...
        Twilio.Chat.Client.create(token).then(function (client) {
            ...
            checkInternet('connected', selector);
            // On connection state change [Updating] and get [info & msgs]
            console.log('Invited to channel000');
    ...
    $("body").on("click", ".messenger-list-item", function () {
        ...
        IDinfo(messenger.split("_")[1], messenger.split("_")[0]);
        channelName = `${authId}-${messenger.split("_")[1]}`;
        channelNameReverse = `${messenger.split("_")[1]}-${authId}`;
        console.log(channelName, channelNameReverse);
        createOrJoinChannel(channelName, channelNameReverse);

    });

    function createOrJoinChannel(channelName, channelNameReverse) {
        // Get the chat channel, which is where all the messages are
        // sent in this simple application
        console.log('Attempting to join  chat channel...');
        console.log(channelName, channelNameReverse);
        setTimeout(function () {
            console.log('channelName', channelName);
            messagingClient.getChannelByUniqueName(channelName)
                ...
                .catch(function () {
                    console.log('channelNameReverse', channelNameReverse);
                    messagingClient.getChannelByUniqueName(channelNameReverse)
                        .then(function (channel) {
                            selectedChannel = channel;
                            console.log(000, selectedChannel);
                            setupChannel();
                        }).catch(function () {
    ...
    function setupChannel() {
        ...
        selectedChannel.join().then(function (channel) {
            ...
            //invite a user to a newly created private chat channel 
            console.log(messenger.split("_")[1], 'invited');
            selectedChannel.invite(messenger.split("_")[1]).then(function () {
                console.log('Your friend has been invited!');
            }).catch(function () {
                console.log('second user already invited');
    ...
    function initChannelEvents() {
        console.log(selectedChannel.friendlyName + ' ready.');
        selectedChannel.on('messageAdded', Messagereceived);
        selectedChannel.on('typingStarted', showTypingStarted);
        selectedChannel.on('typingEnded', hideTypingStarted);
        // selectedChannel.on('memberJoined', notifyMemberJoined);
        // selectedChannel.on('memberLeft', notifyMemberLeft);
        // $inputText.prop('disabled', false).focus();
    }
    //Listen to messages, and append if data received
    function Messagereceived(data) {
        if (data.author == messenger.split("_")[1]) {
            // remove message hint
            $(".message-hint").remove();
            // append message
            console.log((data.body));
            var newMessegeDiv = `<div class="message-card" data-id="2424990837">
            <p>${data.body}
                <sub title="${data.dateCreated}">1 second ago</sub>
                
                    </p>
             </div>`;
            messagesContainer.find(".messages").append(newMessegeDiv);
            console.log('Messagereceived', messenger.split("_")[1], data.author);
    ...
    function showTypingStarted(data) {
        console.log('showTypingStarted');
        makeSeen(true);
        messagesContainer.find(".typing-indicator").addClass('active');
        // scroll to bottom
        scrollBottom(messagesContainer);
    ...
    function hideTypingStarted(member) {
        console.log('hideTypingStarted');
        messagesContainer.find(".typing-indicator").removeClass('active');
        scrollBottom(messagesContainer);
    }
    ...
    $("body").on("click", ".favorite-list-item", function () {
        ...
        IDinfo(messenger.split("_")[1], messenger.split("_")[0]);
        selectedChannel = `${authId}-${messenger.split("_")[1]}`;
        channelName = `${authId}-${messenger.split("_")[1]}`;
        channelNameReverse = `${messenger.split("_")[1]}-${authId}`;
        console.log(channelName, channelNameReverse);
        createOrJoinChannel(channelName, channelNameReverse);
    ...
    $("#message-form .m-send").on("keydown", () => {
        // Trigger typing
        let triggered = selectedChannel.typing();
        triggered
